...
public static void Main(string[] args)
{
    ...
    if (workingConnectionString == null)
    {
        ...
        throw new InvalidOperationException(
            "CRITICAL: Failed to connect to both primary and fallback databases. Application startup aborted.");
    }
...
var primaryEndpoint = GetConfig("MINIO_ENDPOINT")?.Trim();
var primaryAccessKey = GetConfig("MINIO_ACCESS_KEY")?.Trim();
var primarySecretKey = GetConfig("MINIO_SECRET_KEY")?.Trim();
var primaryUseSSL = string.Equals(GetConfig("MINIO_USE_SSL"), "true", StringComparison.OrdinalIgnoreCase);
    ...
    !string.IsNullOrWhiteSpace(primarySecretKey))
{
    try
    {
        ...
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(5));
        testClient.ListBucketsAsync(cts.Token).GetAwaiter().GetResult();
    ...
    Console.WriteLine($"Using Primary MinIO configuration: {primaryEndpoint}");
    TryMinioHealthCheck(primaryEndpoint, primaryAccessKey, primarySecretKey, primaryUseSSL, "Primary");
}
else
{
    Console.WriteLine("Primary MinIO configuration incomplete, trying local fallback...");
}
...
if (string.IsNullOrWhiteSpace(minioEndpoint))
{
    ...
    Console.WriteLine($"Using Fallback MinIO configuration: {fallbackEndpoint}");
    TryMinioHealthCheck(fallbackEndpoint, fallbackAccessKey, fallbackSecretKey, false, "Fallback");
}
    ...
    string.IsNullOrWhiteSpace(minioSecretKey))
{
    ...
    throw new InvalidOperationException(
        "CRITICAL: MinIO configuration is empty. Set MINIO_ENDPOINT, MINIO_ACCESS_KEY and MINIO_SECRET_KEY, or use the local fallback.");
}
