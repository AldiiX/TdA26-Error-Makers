    ...
    middleware: [
        defineNuxtRouteMiddleware(async (to) => {
            ...
            try {
                ...
                // pokud se jedna o draft/archived kurz a uzivatel neni admin ani autor, presmeruje na /courses
                if (["draft", "archived"].includes(course.status) && !(isAdmin || isAuthor)) {
                    return navigateTo("/courses");
                }
            } catch (e) {
                                    ...
                                    <div :class="$style.unassignedSection">
                                        ...
                                        <TransitionGroup tag="ul" name="module-list" :class="{ [$style.isDragging]: draggedModuleUuid }">
                                            ...
                                            >
                                                <Transition name="drag-placeholder">
                                                    <div
                                                        v-if="dragOverModuleUuid === mod.uuid && draggedModuleUuid !== mod.uuid"
                                                        :class="$style.dragPlaceholder"
                                                    />
                                                </Transition>
...
.course {
    ...
    .status {
        ...
        &[data-status="scheduled"] {
            ...
            background: var(--accent-color-additional-1-bg);
        }
        ...
        &[data-status="paused"] {
            ...
            background: var(--accent-color-additional-3-bg);
        }

        // Archived 
        &[data-status="archived"] {
            color: var(--accent-color-additional-4);
            background: var(--accent-color-additional-4-bg);
        }
    }
